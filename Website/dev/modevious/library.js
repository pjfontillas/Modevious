...
return null;},trace:function(message){$c.log[$c.log.length]=[$c.getTime(),": ",message].join('');$j("#console_text").append(["<p>",$c.log[$c.log.length-1],"</p>"].join(''));return message;},showLog:function(event){$j.pnotify(event.keyCode);if(event.keyCode==$c.config.consoleCode[$c.consoleCounter]){$c.consoleCounter++;}else{$c.consoleCounter=0;}
