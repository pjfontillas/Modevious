...
$c.console.warn(["There was a problem sending the log. Here is the response:",transport.status,transport.statusText].join(' '));}});},show:function(){$j("#modevious_console").css({position:"fixed"}).animate({top:0});},hide:function(){$j("#modevious_console").animate({top:(($j("#modevious_console").height()+500)*-1)+"px"});},checkCode:function(event){if(event.keyCode==$c.config.consoleCode[$c.consoleCounter]){$c.consoleCounter++;}else{$c.consoleCounter=0;}
if($c.consoleCounter==$c.config.consoleCode.length){$c.console.show();}}},getFlashMovie:function(movieName){var isIE=navigator.appName.indexOf("Microsoft")!=-1;return(isIE)?window[movieName]:document[movieName];},update:function(name,version,url){$c.updateName[updateName.length]=name;$c.updateVersion[updateVersion.length]=version;$c.updateURL[updateURL.length]=url;}};}());$c.init();try{$c.update("Modevious",$c.getVersion(),"http://modevious.com/js/update.php");}catch(e){}/*!
 * jQuery UI 1.8.10
 *
...
$c.include($c.config.jQueryUIThemeURL);$c.include($c.config.libraryURL);soundManager.url=$c.config.soundManager.url;soundManager.flashVersion=$c.config.soundManager.flashVersion;var stack_topleft={"dir1":"down","dir2":"right","firstpos1":15,"firstpos2":15};var stack_bottomleft={"dir1":"up","dir2":"right","firstpos1":15,"firstpos2":15};var stack_bottomright={"dir1":"up","dir2":"left","firstpos1":15,"firstpos2":15};$c.onLoad(function(){$j("body").append(["<div id=\"modevious_console\">","<div id=\"modevious_console_top\">","<div id=\"modevious_minimize_console_button\"></div>","</div>","<div id=\"modevious_console_middle\">","<div id=\"modevious_console_text\"></div>","</div>","<div id=\"modevious_console_bottom\"></div>","</div>"].join(''));$j("#modevious_minimize_console_button").click($c.console.hide);if(document.addEventListener){document.addEventListener("keydown",function(event){$c.console.checkCode(event);},false);}else{document.attachEvent("onkeydown",function(event){$c.console.checkCode(event);});}
$j("#modevious_console").draggable({handle:"#modevious_console_top, #modevious_console_bottom"}).css("position","fixed");$j(".tabs").tabs();$j(".accordion").accordion({header:"h3",autoHeight:false,collapsible:true});$j(":button").button();$j(".draggable").draggable({cursor:"move",cancel:"p, img, h1, h2, h3, h4, h5, a"});$j(".resizable").resizable();if(typeof(window.console)!="undefined"){console.log("jQuery User Interface initialized.");}
