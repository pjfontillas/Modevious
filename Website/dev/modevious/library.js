...
var $j=jQuery.noConflict();var $c=(function(){var version=122;var versionString="v1.2.2";var config={warnings:false,at:"(AT)",dot:"(DOT)",soundManager:{url:"/modevious/swf",flashVersion:9},consoleCode:[Event.KEY_UP,Event.KEY_UP,Event.KEY_DOWN,Event.KEY_DOWN,Event.KEY_LEFT,Event.KEY_RIGHT,Event.KEY_LEFT,Event.KEY_RIGHT,66,65],modeviousLocation:"/modevious/",jQueryUIThemeURL:"/modevious/jquery-ui-1.8.5.custom.css",libraryURL:"/modevious/library.css"};var loadedScripts=[];var log=[];var updateName=[];var updateVersion=[];var updateURL=[];var updateCounter=0;var consoleCounter=0;return{config:config,loadedScripts:loadedScripts,log:log,consoleCounter:consoleCounter,updateName:updateName,updateVersion:updateVersion,updateURL:updateURL,updateCounter:updateCounter,updateFound:false,init:function(){if(window.$config!==undefined){if(window.$config.warnings!==undefined){$c.config.warnings=window.$config.warnings;}
...
return null;},eraseCookie:function(name){$c.createCookie(name,'',-1);},addLoadEvent:function(func){var oldonload=window.onload;if(typeof window.onload!=="function"){window.onload=func;}else{window.onload=function(){if(oldonload){oldonload();}
func();};}},onLoad:function(func){$c.addLoadEvent(func);},getUrlVariable:function(urlVariable){var urlArray=[];var tempUrlArray=[];tempUrlArray=location.href.split('?');tempUrlArray[1].replace("&",'&');urlArray=tempUrlArray[1].split('&');for(var arrayPos=0;arrayPos<urlArray.length;arrayPos++){tempUrlArray=urlArray[arrayPos].split('=');if(tempUrlArray[0]===urlVariable){return tempUrlArray[1];}}
...
return null;},trace:function(message){$c.log[$c.log.length]=[$c.getTime(),": ",message].join('');$j("#console_text").append(["<p>",$c.log[$c.log.length-1],"</p>"].join(''));return message;},showLog:function(event){if(event.keyCode==$c.config.consoleCode[$c.consoleCounter]){$c.consoleCounter++;}else{$c.consoleCounter=0;}
if($c.consoleCounter==$c.config.consoleCode.length){$j("#console").animate({top:document.viewport.getScrollOffsets().top});}},hideLog:function(){$j("#console").animate({top:"-500"});},getFlashMovie:function(movieName){var isIE=navigator.appName.indexOf("Microsoft")!=-1;return(isIE)?window[movieName]:document[movieName];},update:function(name,version,url){$c.updateName[updateName.length]=name;$c.updateVersion[updateVersion.length]=version;$c.updateURL[updateURL.length]=url;}};}());$c.init();try{$c.update("Modevious",$c.getVersion(),"http://modevious.com/js/update.php");}catch(e){}/*!
 * jQuery UI 1.8.5
 *
