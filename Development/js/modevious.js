...
var $j=jQuery.noConflict();var $c=(function(){var version=110
var versionString="v1.1.0"
var config={warnings:false,at:"(AT)",dot:"(DOT)",update:false,soundManager:{url:"/swf",flashVersion:9}}
var loadedScripts=[]
var log=[]
var konamiCounter=0
var updateName=[]
...
var updateURL=[]
var updateCounter=0
return{version:version,versionString:versionString,config:config,loadedScripts:loadedScripts,log:log,konamiCounter:konamiCounter,updateName:updateName,updateVersion:updateVersion,updateURL:updateURL,updateCounter:updateCounter,updateFound:false,init:function(){try{$c.config=$config;}catch(e){}
return true;},changePage:function(pageName,objectID){$c.fetchData(pageName,null,objectID);return true;},createCookie:function(name,value,days){var date=new Date();var expires;if(days){date.setTime(date.getTime()+(days*24*60*60*1000));expires=["; expires=",date.toGMTString()].join('');}else{expires='';}
document.cookie=[name,'=',value,expires,"; path=/"].join('');return true;},readCookie:function(name){var nameEQ=[name,'='].join('');var ca=document.cookie.split(';');for(var i=0;i<ca.length;i++){var c=ca[i];while(c.charAt(0)===' '){c=c.substring(1,c.length);}
...
if(!loaded){$c.loadedScripts[$c.loadedScripts.length]=scriptID;var ext=scriptID.substr((scriptID.lastIndexOf('.')+1),scriptID.length).toLowerCase();switch(ext){case"js":document.write(["<script type=\"text/javascript\" src=\"",scriptID,"\"></script>"].join(''));break;case"css":document.write(["<link type=\"text/css\" href=\"",scriptID,"\" rel=\"stylesheet\" />"].join(''));break;default:alert(["Unsupported filetype: .",ext].join(''));}}
return true;},add2Lib:function(scriptID){$c.loadedScripts[$c.loadedScripts.length]=scriptID;return true;},exclude:function(scriptID){$c.add2Lib(scriptID);return true;},showEmail:function(){var url;$$(".email").each(function(a){url=a.href.replace($c.config.at,'@');url=url.replace($c.config.dot,'.');a.href=url;url=url.replace("mailto:",'');a.update(url);}.bind(this));return true;},getTime:function(){var a_p="";var d=new Date();var curr_hour=d.getHours();if(curr_hour<12){a_p="AM";}else{a_p="PM";}
...
$j(this).css({"left":[newLeft,"px"].join('')});}});return true;},vAlignThis:function(element){var parentHeight;var thisHeight;var newTop;if(element.hasClass("vAlignWindow")){parentHeight=document.viewport.getHeight();thisHeight=element.outerHeight(true);newTop=(parentHeight-thisHeight)/2;if(newTop<1){newTop=0;}
...
element.css({"position":"relative","top":[newTop,"px"].join('')});}
return true;},setActiveStyleSheet:function(title){var i,a,main;for(i=0;(a=document.getElementsByTagName("link")[i]);i++){if(a.getAttribute("rel").indexOf("style")!==-1&&a.getAttribute("title")){a.disabled=true;if(a.getAttribute("title")===title){a.disabled=false;}}}
$c.createCookie("style",title,365);return true;},getActiveStyleSheet:function(){var i,a;for(i=0;(a=document.getElementsByTagName("link")[i]);i++){if(a.getAttribute("rel").indexOf("style")!==-1&&a.getAttribute("title")&&!a.disabled){return a.getAttribute("title");}}
return null;},trace:function(message){$c.log[$c.log.length]=[$c.getTime(),": ",message].join('');$j("#console_text").append(["<p>",$c.log[$c.log.length-1],"</p>"].join(''));return true;},showLog:function(event){switch($c.konamiCounter){case 0:if(event.keyCode==Event.KEY_UP){$c.konamiCounter++;}else{$c.konamiCounter=0;}
break;case 1:if(event.keyCode==Event.KEY_UP){$c.konamiCounter++;}else{$c.konamiCounter=0;}
break;case 2:if(event.keyCode==Event.KEY_DOWN){$c.konamiCounter++;}else{$c.konamiCounter=0;}
break;case 3:if(event.keyCode==Event.KEY_DOWN){$c.konamiCounter++;}else{$c.konamiCounter=0;}
break;case 4:if(event.keyCode==Event.KEY_LEFT){$c.konamiCounter++;}else{$c.konamiCounter=0;}
break;case 5:if(event.keyCode==Event.KEY_RIGHT){$c.konamiCounter++;}else{$c.konamiCounter=0;}
break;case 6:if(event.keyCode==Event.KEY_LEFT){$c.konamiCounter++;}else{$c.konamiCounter=0;}
break;case 7:if(event.keyCode==Event.KEY_RIGHT){$j("#console").animate({top:document.viewport.getScrollOffsets().top});}else{}
$c.konamiCounter=0;return true;break;default:$c.konamiCounter=0;}
return false;},hideLog:function(){$j("#console").animate({top:"-500"});return true;},getFlashMovie:function(movieName){var isIE=navigator.appName.indexOf("Microsoft")!=-1;return(isIE)?window[movieName]:document[movieName];},update:function(name,version,url){$c.updateName[updateName.length]=name;$c.updateVersion[updateVersion.length]=version;$c.updateURL[updateURL.length]=url;},fetchUpdates:function(){var data,updateType;if($c.config.update){$j("body").css("cursor","progress");$c.trace(["Fetching update from: ",$c.updateURL[updateCounter]].join(''));var url=[$c.updateURL[updateCounter],"?callback=?"].join('');$j.getJSON(url,function(data){if(parseInt(data.version)>$c.updateVersion[$c.updateCounter]){if(data.type=="Critical"){updateType="ui-state-error";}else{updateType="ui-state-highlight";}
$j("#update_manager").append(["<div class=\"update ",updateType,"\">","<p><strong>",$c.updateName[updateCounter],"</strong> | <small>",data.versionString,"</small></p>","<p>",data.description,"</p>","<p><a href=\"",data.url,"\">Download this update</a>"," | <a href=\"",data.changelog,"\">View the changelog</a></p>","</div>"].join(''));$c.updateFound=true;$c.trace(["Found an update for: ",$c.updateName[updateCounter]].join(''));}
$c.updateCounter++;if($c.updateCounter!=$c.updateURL.length){return $c.fetchUpdates();}else{if($c.updateFound){$j("#progress").hide("blind");}else{$j("#progress").html("Everything is up-to-date!");}
$j("body").css("cursor","auto");return true;}});}
return false;}};}());$c.init();try{$c.update("Modevious",110,"http://modevious.com/js/update.php");}catch(e){}
